...
class TError {
    ...
    translate(text) {
        if (typeof this.constructor.errors !== 'undefined' && typeof this.constructor.errors[text] !== 'undefined') {
            let translatedText = this.constructor.errors[text]
            if (arguments.length > 1) {
                // message has to be parsed
                const elements = arguments
                translatedText = translatedText.replace(/{(\d+)}/g, (match, number) => {
...
TError.loadMessages = callback => {
    // Load translated errors
    const errorsFile = TEnvironment.getResource('errors.json')
    const language = TEnvironment.getLanguage()
